...
resource "aws_apigatewayv2_api" "hello_vpc_api" {
  ...
  cors_configuration {
    # Allow both local UI and the CloudFront-hosted UI origin.
    allow_origins = distinct(concat(var.allowed_origins, ["https://${aws_cloudfront_distribution.ui.domain_name}"]))
...
resource "aws_cognito_user_pool_client" "main" {
  ...
  callback_urls = [
    ...
    "https://${aws_cloudfront_distribution.ui.domain_name}/callback.html",
  ]
  logout_urls = [
    "http://localhost:3000/logout.html",
    "https://${aws_cloudfront_distribution.ui.domain_name}/logout.html",
  ]
...
resource "random_id" "ui_bucket_suffix" {
  byte_length = 3
}

locals {
  ui_bucket_name = "${var.project_name}-ui-${var.environment}-${random_id.ui_bucket_suffix.hex}"
  ui_dir         = "${abspath(path.module)}/../ui/hosted_ui_local"
  ui_static_files = setsubtract(
    fileset(local.ui_dir, "*"),
    ["config.js"]
  )
  ...
  ui_config_js = <<-EOT
    // Generated by Terraform. Do not edit in S3 directly.
    window.APP_CONFIG = {
      cognitoBaseUrl: "https://${aws_cognito_user_pool_domain.hosted_ui.domain}.auth.${var.aws_region}.amazoncognito.com",
      clientId: "${aws_cognito_user_pool_client.main.id}",
      redirectUri: "https://${aws_cloudfront_distribution.ui.domain_name}/callback.html",
      logoutRedirectUri: "https://${aws_cloudfront_distribution.ui.domain_name}/logout.html",
      apiProtectedEndpoint: "${aws_apigatewayv2_api.hello_vpc_api.api_endpoint}/protected",
...
resource "aws_s3_object" "ui_static" {
  for_each = local.ui_static_files
  ...
  content_type = lookup(local.ui_content_types, element(reverse(split(".", each.value)), 0), "binary/octet-stream")
}
...
resource "aws_s3_object" "ui_config" {
  ...
  content      = local.ui_config_js
  etag         = md5(local.ui_config_js)
